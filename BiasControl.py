...
class BiasChannel():
    ...
    def set_pot(self, pot=1, wiper=0, value=0):
        ...
        if pot == 2:
            ...
            pot = self.get_fpot1()
        write16(pot, AD5144_CMD_WRITE_RDAC+wiper, value)
        self.__end([pot])
...
# INA219 stuff below...
    def ina_init_all(self):
        ...
        """
        self.__start()
        self.fina219s = [self.get_fina(dev) for dev in range(8)]
        def set_calibration(fd):
            ina219_calValue = 4096
            self.ina219_currentDivider_mA = 10 # Current LSB = 100uA per bit (1000/100 = 10)
            
            self.ina219_powerMultiplier_mW = 2 # Power LSB = 1mW per bit (2/1)

            # Set Calibration register to 'Cal' calculated above
            write16(fd, INA219_REG_CALIBRATION, ina219_calValue)
            ...
            self.inaconfig = config
            self.ina219_calValue = ina219_calValue
            write16(fd, INA219_REG_CONFIG, config)
        
        for i in self.fina219s:
            set_calibration(i)

        self.__end(self.fina219s)

    def ina_getCurrent_raw(self, chan):
        self.__start()
        ina = self.get_fina(chan)
        write16(ina, INA219_REG_CALIBRATION, self.ina219_calValue)
